...
class Recording(object):
    ...
    def __init__(self, dir_to_save, name_to_save,
                 continue_experiment, save_images):
        ...
        if not os.path.exists('_benchmarks_results/'+dir_to_save):
            os.mkdir('_benchmarks_results/'+dir_to_save)

        # Generate the full path for the log files
        self._path = os.path.join('_benchmarks_results', dir_to_save
                                  , name_to_save
                                  )
        ...
        self._image_filename_format = os.path.join(
            self._path, '_images/image_{:0>5d}')
    ...
    def _continue_experiment(self, continue_experiment):
        ...
        def get_non_existent_path(f_name_path):
            ...
            """
            ## doing this to counter a TCPConnectionError reset error.
            if not os.path.exists(f_name_path):
                return f_name_path
            else:
                os.system("rm -rf " + f_name_path)
                return f_name_path
    ...
    def save_images(self, sensor_data, episode_name, frame):
        ...
        if self._save_images:
            for name, image in sensor_data.items():
                image.save_to_disk(self._image_filename_format.format(frame))
